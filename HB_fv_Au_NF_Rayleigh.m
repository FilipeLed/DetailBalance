...
%% Loop (volumetric fraction dependence)
%%
fv = 0:2/1e5:1/1e3;                               %volume fraction vector[%]

for n = 1:length(fv)
%% 6° STEP
%THERMO-PHYSICAL PROPERTIES OF NANOFLUIDS

[A_nf,p_nf,k_nf,Cp_nf,h_g_nf,u_nf]=ThermoPhysical(fv(n),p_p,p_w,Dvis_w,k_w,k_p,Tamb,D,Cp_p,Cp_w,m_flow,t_nf,W_nf,L_nf);
...
Row1 = {'VF', 'P_el', 'P_th', 'Tout', 'ef_el','ef_th'};
%directoryname = uigetdir('', 'Escolha uma pasta para salvar o arquivo');
file_name = fullfile(directoryname,'Artigo_Rayleigh.xlsx');
xlswrite(file_name,Row0,'FV','G1');
xlswrite(file_name,Row1,'FV','G2');
xlswrite(file_name,fv','FV','G3');
xlswrite(file_name,P_el','FV','H3');
xlswrite(file_name,P_th','FV','I3');
xlswrite(file_name,Tout','FV','J3');
xlswrite(file_name,ef_el','FV','K3');
xlswrite(file_name,ef_th','FV','L3');
