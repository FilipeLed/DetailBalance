...
t_nf = 10*10^-3;                                    %nanofluid thickness [m]
...
%% Loop (Diameter dependence)
%%
D = 1/1e6:1/10:50.1;                                    %nanoparticle diameter vector[nm]
...
[A_nf,p_nf,k_nf,Cp_nf,h_g_nf,u_nf]=ThermoPhysical(fv,p_p,p_w,Dvis_w,k_w,k_p,Tamb,D(n),Cp_p,Cp_w,m_flow,t_nf,W_nf,L_nf);

%% 6º STEP
%NANOFLUID OPTICAL CHARACTERIZATION

[Qext,Qsca,Qabs,m]=RayleighScattering(wl_solar,D(n),subplus(nw(wl_solar)),complex(np(wl_solar),kp(wl_solar)));
...
ke = 3*fv*Qext./(2*D(n)*10^-9);                                     %Extinction coefficient of the nanoparticle
...
False = 0;
xlCenter = -4108;
Row0 = {'Gold',NaN,NaN,NaN,NaN};
Row1 = {'D', 'P_el', 'P_th', 'Tout', 'ef_el','ef_th'};
%directoryname = uigetdir('', 'Escolha uma pasta para salvar o arquivo');
file_name = fullfile(directoryname,'Artigo_Rayleigh.xlsx');
xlswrite(file_name,Row0,'Diameter','G1');
xlswrite(file_name,Row1,'Diameter','G2');
xlswrite(file_name,D','Diameter','G3');
xlswrite(file_name,P_el','Diameter','H3');
xlswrite(file_name,P_th','Diameter','I3');
xlswrite(file_name,Tout','Diameter','J3');
xlswrite(file_name,ef_el','Diameter','K3');
xlswrite(file_name,ef_th','Diameter','L3');
