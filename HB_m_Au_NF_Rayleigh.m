...
%% Loop (mass flow rata dependance)

m_flow = 0:0.0002:0.02;                             %nanofluid mass flow rate Kg/s
...
[A_nf,p_nf,k_nf,Cp_nf,h_g_nf,u_nf]=ThermoPhysical(fv,p_p,p_w,Dvis_w,k_w,k_p,Tamb,D,Cp_p,Cp_w,m_flow(n),t_nf,W_nf,L_nf);
...
F = @(x)[(-x(1)+q*g((Egap0 - (Alfa*x(3)^2)/(x(3) + beta)))-(K*x(3)^3/n2*(exp(-q*(Egap0 - (Alfa*x(3)^2)/(x(3) + beta))/(n3*kb*x(3)))))*(exp((q*(x(2)+x(1)*4.6*1e-08*exp(0.0207*x(3))))/(n1*kb*x(3)))-1)-(x(2)+x(1)*4.6*1e-08*exp(0.0207*x(3)))/153.92*1e-04*exp(799.93/x(3)))
  ...
  P_el(n) = x(1)*x(2);
  P_th(n) = ((m_flow(n)*Cp_nf/(A_nf))*(x(5)-Tin));
  ef_el(n) = x(1)*x(2)/trapz(wl_solar,radiation);
  ef_th(n) = ((m_flow(n)*Cp_nf/(A_nf))*(x(5)-Tin))/trapz(wl_solar,radiation);
  Tout(n) = x(5);
...
Row1 = {'m_flow', 'P_el', 'P_th', 'Tout', 'ef_el','ef_th'};
%directoryname = uigetdir('', 'Escolha uma pasta para salvar o arquivo');
file_name = fullfile(directoryname,'Artigo_Rayleigh.xlsx');
xlswrite(file_name,Row0,'mass flow','G1');
xlswrite(file_name,Row1,'mass flow','G2');
xlswrite(file_name,m_flow','mass flow','G3');
xlswrite(file_name,P_el','mass flow','H3');
xlswrite(file_name,P_th','mass flow','I3');
xlswrite(file_name,Tout','mass flow','J3');
xlswrite(file_name,ef_el','mass flow','K3');
xlswrite(file_name,ef_th','mass flow','L3');
